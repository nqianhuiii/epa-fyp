...
const TextbookViewerScreen = () => {
  ...
  const fetchTextbookById = async (id: string) => {
    try {
      setInitialLoading(true);
      const result = await controller.getTextbookById(id);
      if (result) {
        setTextbook(result);
      } else {
        setError('Buku teks tak jumpa');
      }
    } catch (err) {
      console.log('Error fetching textbook:', err);
      setError('Gagal memuatkan buku teks');
    } finally {
  ...
  const handleDownload = async () => {
    if (!textbook?.pdfUrl) return;

    setDownloading(true);
    setDownloadProgress(0);

    const fileName = `${textbook.title.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;

    await downloadAndHandleFile(textbook.pdfUrl, fileName, setDownloadProgress);
...
export default TextbookViewerScreen;
